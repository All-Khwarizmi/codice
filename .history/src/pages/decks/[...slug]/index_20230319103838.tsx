                ...
                <div className="text-white max-w-xs lg:max-w-sm" key={deck._id}>
                  <Link href={`./decks/${deck.name}`}>
                    <img
                      ...
                      alt={`${deck.name} image`}
                    />
                    <div className="bg-gray-700  rounded-lg rounded-t-none">
                      <p className="uppercase font-bold p-5 pb-3 text-md">{deck.name}</p>
                      <p className="pl-5 italic text-gray-400 font-bold text-sm">{deck.description}</p>
                      <p className="pl-5 text-sm  bg-black py-5"> {deck.difficulty?.name} </p>
                    </div>
                  </Link>
                </div>
              );
