                ...
                <div className="text-white max-w-xs lg:max-w-sm" key={deck._id}>
                  <Link href={`./decks/${deck.name}`}>
                    <img
                      ...
                      alt={`${deck.name} image`}
                    />
                    <div className="bg-gray-700 min-h-md rounded-lg rounded-t-none">
                      <p className="uppercase font-bold p-5 pb-3 text-md">{deck.name}</p>
                      <p className="pl-5 italic text-gray text-sm">{deck.description}</p>
                      <p className="pl-5 text-sm pt-8"> {deck.difficulty?.name} </p>
                    </div>
                  </Link>
                </div>
              );
