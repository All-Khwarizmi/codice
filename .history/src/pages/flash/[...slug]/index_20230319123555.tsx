...
const FlashCard = ({ data }: FlashData) => {
  ...
  const dataRef = useRef(data.map((item) => item.flashCard))
  const flashCards = useRef(
    data.map((item) => item.flashCard.map((flashCard) => flashCard).length.toString())
  );
  const numberOfQuestion = flashCards.current;
 
  console.log(
    data,
    "FlashCards",
    flashCards,
    "data.map((item) => item.flashCard)",
    data.map((item) => item.flashCard.map((flashCard) => flashCard).length.toString().toString()),
